...
const LeagueItem = styled.div`
	display: flex;
	align-items: center;
	background-color: ${({ theme }: { theme: { background: string; neutral3: string } }) => theme.background};
	border: 1px solid ${({ theme }: { theme: { background: string; neutral3: string } }) => theme.neutral3};
	border-radius: 10px;
	padding: 15px;
	margin: 10px 0;
	cursor: pointer;
...
const Button = styled.button`
	padding: 12px 24px;
	background-color: ${({ theme }: { theme: { neutral3: string; background: string } }) => theme.neutral3};
	border: none;
	border-radius: 5px;
...
function LeaguePicker(): React.ReactElement {
	...
	return (
		<Container>
			...
			<h3>Add a League</h3>
			<Button onClick={() => navigate("/sleeper-login")}>
				Add Sleeper League
			</Button>
			<Button onClick={() => navigate("/espn-login")}>
				Add ESPN League
			</Button>
		</Container>
	);

...
/**
 * Build a map from ESPN team ID → member display name.
 *
 * @param data - ESPN league response containing teams and members arrays
 * @returns Map of teamId → owner display name
 */
function buildOwnerMap(data: ESPNLeagueResponse): Map<number, string> {
	const memberMap = new Map<string, string>();
	for (const m of data.members ?? []) {
		memberMap.set(m.id, m.displayName);
	}
	const ownerMap = new Map<number, string>();
	for (const t of data.teams ?? []) {
		ownerMap.set(t.id, memberMap.get(t.primaryOwner) ?? "Unknown Owner");
	}
	return ownerMap;
}
...
function buildRosterPositions(data: ESPNLeagueResponse): string[] {
	...
	if (!slotCounts) return ["QB", "RB", "RB", "WR", "WR", "TE", "FLEX", "K", "DEF", "BN", "BN", "BN", "BN", "BN", "BN"];
...
export async function getBracketMatchups(
	...
	for (const item of playoffItems) {
		...
		brackets.push({
			...
			w: item.winner === "HOME" ? (item.home?.teamId ?? null) :
				item.winner === "AWAY" ? (item.away?.teamId ?? null) : null,
			l: item.winner === "HOME" ? (item.away?.teamId ?? null) :
				item.winner === "AWAY" ? (item.home?.teamId ?? null) : null,
		});
	}

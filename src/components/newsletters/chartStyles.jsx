...
export const ShotsDistributionChart = ({ chartData }) => {
  ...
  return (
    ...
    >
      ...
      />
      <VictoryAxis dependentAxis tickCount={7} {...getAxisConfigWithGrid(theme, { grid: true })} />
      <VictoryStack colorScale="qualitative">
...
export const WeeklyScoringChart = ({ chartData }) => {
  ...
  return (
    <VictoryChart {...getBaseChartConfig()}>
      ...
      <VictoryScatter data={data} x="week" y="team_points" style={{ data: { fill: theme.text } }} />
    </VictoryChart>
  );
};
...
export const MatchupPlot = ({ data, matchupId }) => {
  ...
  const uniquePositions = positionOrder
    .filter((position) =>
      filteredData.some((team) => team.entries.some((entry) => entry.position === position))
    )
    .reverse();

  const totalPointsByTeam = filteredData.map((team) => ({
    team_name: team.team_name,
    totalPoints: parseFloat(team.entries.reduce((sum, entry) => sum + entry.points, 0).toFixed(2)),
  }));
...
export const CustomDataComponent = (props) => {
  ...
  return (
    <g transform={`translate(${x}, ${y})`}>
      <image x="-15" y="-15" width="30" height="30" href={datum.image_or_text} alt="" />
    </g>
  );
};
...
export const PfPaScatter = ({ leaderboardData }) => {
  ...
  const cornerLabels = [
    {
      lines: ["Unlucky", "and", "Bad"],
      colors: [theme.newsRed, theme.text, theme.newsRed],
      x: minPF,
      y: maxPA,
      dx: 10,
      dy: 20,
    ...
    {
      lines: ["Unlucky", "and", "Good"],
      colors: [theme.newsRed, theme.text, theme.newsBlue],
      x: maxPF,
      y: maxPA,
      dx: -10,
      dy: 20,
    ...
    {
      lines: ["Lucky", "and", "Bad"],
      colors: [theme.newsBlue, theme.text, theme.newsRed],
      x: minPF,
      y: minPA,
      dx: 10,
      dy: 10,
    ...
    {
      lines: ["Lucky", "and", "Good"],
      colors: [theme.newsBlue, theme.text, theme.newsBlue],
      x: maxPF,
      y: minPA,
      dx: -10,
      dy: 10,
...
export const LineupScatter = ({ data }) => {
  ...
  // Calculate medians
  const medianStrength = data.map((d) => d.strength).sort((a, b) => a - b)[
    Math.floor(data.length / 2)
  ];
  const medianBalance = data.map((d) => d.balance).sort((a, b) => a - b)[
    Math.floor(data.length / 2)
  ];

  const cornerLabels = [
    {
      lines: ["Consistent", "and", "Bad"],
      colors: [theme.newsBlue, theme.text, theme.newsRed],
      x: minStrength,
      y: maxBalance,
      dx: 30,
...
export const TradesLineChart = ({ tradeHistory }) => {
  ...
  return (
    <VictoryChart {...getBaseChartConfig()}>
      ...
      <VictoryLegend {...getBaseLegendConfig(theme, 75)} data={legendData} />
  ...
  return (
    ...
    >
      ...
      <VictoryScatter
        ...
        labelComponent={
          <VictoryLabel
            dx={({ datum }) => (datum.avg_opponent_diff > 0 ? 25 : -25)}
            dy={0}
            style={{ fontSize: 12, fill: theme.text }}
          />
        }
